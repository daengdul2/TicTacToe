...
export default function Home() {
            ...
            if (snap.exists()) {
                ...
                setRoomsList(
                    Object.entries(data).map(([id, room]) => ({
                        ...
                        playersCount: [room.playerX, room.playerO].filter(
                            Boolean
                        ).length
                    }))
                );
    ...
    return (
        <main>
            ...
            <div>
                User: {user ? user.uid.substring(0, 8) : "... signing in"}
            </div>
            ...
            <div>
                ...
                <button
                    onClick={() => createRoom(customRoomId, preferredSymbol)}>
                    Create Room
                </button>
            </div>
            ...
            <div>
                <button onClick={() => botGame()}>Com</button>
            </div>
        </main>
    );
}
